...
int main()
{
    ...
    PilaPacientes pilaEntrada, pilaPacientesTemp; // Pilas de pacientes, tanto la primera donde se introducen los
                                                  // pacientes y la temporal para la consulta 1.1
    string DNI; // Iniciamos el DNI por el que se van a realizar las diversas busquedas.

    Paciente p1, p2, p3, p4, p5, p6, p7, p8, p9, p10,
        pacienteActual; // Generamos 10 pacientes y el paciente que vamos a usar de apoyo
    ...
    while(opcion != 5) {
        ...
        case 0:
            ...
            if(nEmergencia < 1 ||
                nEmergencia >
                    4) { // Controlamos que el usuario no pueda meter una opción que no se muestre por pantalla
            ...
            if(opcionSeleccionTiempo == 1) {
                ...
                cout << "Inserte el anno de ingreso: ";
                cin >> anno;
                cout << "Inserte el mes de ingreso: ";
                cin >> mes;
                cout << "Inserte el dia de ingreso: ";
                cin >> dia;
                cout << "Inserte la hora de ingreso: ";
                cin >> hora;
                cout << "Inserte el minuto de ingreso: ";
                cin >> minuto;
                cout << "Inserte el segundo de ingreso: ";
                cin >> seg;
                ...
                if(!valoresCorrectos) { // Si se ha introducido algun valor incorrecto se vuelve al menu principal y se
        ...
        case 1:
            ...
            do {
                ...
                if(pacienteActual.getDNI() != DNI) { // Si el dni del paciente no coincide con el DNI introducido,se
                                                     // introduce en una pila temporal
            ...
            try {                            // Busca en la lista de emergencia Roja
                ...
                cout << "No se encuentra en la lista roja"
                     << endl; // Si no está en esta lista, salta una excepción y seindica por pantalla
            }
            getch();
            ...
            try {                               // Busca en la lista de emergencia Naranaja
                ...
                cout << "No se encuentra en la lista naranja"
                     << endl; // Si no está en esta lista, salta una excepción y se indica por pantalla
            }
            getch();
            ...
            try {                                // Busca en la lista de emergencia Amarilla
                ...
                cout << "No se encuentra en la lista amarilla"
                     << endl; // Si no está en esta lista, salta una excepción y se indica por pantalla
            }
            getch();
            ...
            try {                             // Busca en la lista de emergencia Verde
                ...
                cout << "No se encuentra en la lista verde"
                     << endl; // Si no está en esta lista, salta una excepción y se indica por pantalla
            }
            getch();
        ...
        case 2:
            ...
            if(emergenciaA < 1 || emergenciaA > 4) { // Se comprueba que la entrada sea entre 1 y 4
                cout << endl
                     << "ERROR: Numero de emergencia antigua erroneo."
                     << endl;                                     // Si hay un error, se indica por pantalla
                cout << "Regresando a menu principal..." << endl; // Regresamos al menú principal break;
            }

            try {
                switch(emergenciaA) { // Dependiendo de la lista en la que se encuentre entrará en un caso, seleccionará
                                      // al paciente y lo borrará de la lista
                case 1:
            ...
            cout << "Paciente: " << pacienteActual.getApell1()
                 << endl; // Se imprime por pantalla el paciente seleccionado
        ...
        case 3:
                ...
                if(opcionP31 == 1) {
                    ...
                    do { // Vamos comparando el DNI introducido hasta encontrar el paciente correspondiente
                        ...
                        if(pacienteActual.getDNI() == DNI31) {  // Comparamos si tienen el mismo DNI
                            cout << endl
                                 << endl
                                 << "El paciente buscado es: "
                                 << endl; // En caso positivo, se imprimer el paciente por pantalla
                    ...
                    do {              // Extraemos el paciente de la pila temporal para reconstruir nuestra pilaEntrada
                        ...
                        !pilaPacientesTemp.estaVacia()); // Esto se repetirá hasta que la pilaPacientesTemp esté vacía
                } else if(opcionP31 == 2) {
            ...
            case 2:
                ...
                if(opcionP32 == 1) {
                    ...
                    cin >> numeroPaciente; // El usuario introduce el codigo que quiere consultar
                    try {
                        pacienteActual =
                            ArbolRojo->buscar(numeroPaciente); // Buscamos el codigo del paciente en la lista Roja
                        if(pacienteActual.getCodNumerico() == numeroPaciente) { // Si coincide con el codigo introducido
                            cout << "DNI del paciente: " << pacienteActual.getDNI()
                                 << " se encuentra en la lista de emergencias roja"
                                 << endl; // Se indica por pantalla que se ha encontrado
                            break;
                        }
                    } catch(...) {
                    }
                    ...
                    try {
                        pacienteActual = ArbolNaranja->buscar(
                            numeroPaciente); // Buscamos el codigo del paciente en la lista Naranja if
                        if(pacienteActual.getCodNumerico() == numeroPaciente) { // Si coincide con el codigo introducido
                            cout << "DNI del paciente: " << pacienteActual.getDNI()
                                 << " se encuentra en la lista de emergencias naranja"
                                 << endl; // Se indica por pantalla que se haencontrado
                            break;
                        }
                    } catch(...) {
                    }
                    ...
                    try {
                        pacienteActual = ArbolAmarillo->buscar(
                            numeroPaciente); // Buscamos el codigo del paciente en la lista Naranja if
                        if(pacienteActual.getCodNumerico() ==
                            numeroPaciente) { // Si coincide con el cÃ³digo introducido
                            cout << "DNI del paciente: " << pacienteActual.getDNI()
                                 << " se encuentra en la lista de emergencias naranja"
                                 << endl; // Se indica por pantalla que se haencontrado
                            break;
                    ...
                    try {
                        pacienteActual =
                            ArbolVerde->buscar(numeroPaciente); // Buscamos el codigo del paciente en la lista Verde
                        if(pacienteActual.getCodNumerico() == numeroPaciente) { // Si coincide con el codigo introducido
                            cout << "DNI del paciente: " << pacienteActual.getDNI()
                                 << " se encuentra en la lista de emergencias verde "
                                 << endl; // Se indica por pantalla que se ha encontrado
                            break;
                        }
                    } catch(...) {
                    ...
                    cout << "El paciente con el codigo numerico: " << numeroPaciente
                         << " no se ha encontrado en ninguna lista de emergencias " << endl;
                    ...
                    cout << "Opcion: ";
                    cin >> ArbolConsultado;
                    cout << endl;             // El usuario introduce la lista de emergencia que desea consultar
            ...
            case 3:
                ...
                while(i < 10) {
                    try {
                        ...
                        if(pt.getCodNumerico() != -1) {
                            pt.imprimeC3(tstruct.tm_year + 1900, tstruct.tm_mon + 1, tstruct.tm_mday, tstruct.tm_hour,
                ...
                while(i < 10) {
                    try {
                        ...
                        if(pt.getCodNumerico() != -1) {
                            pt.imprimeC3(
                                tstruct.tm_year + 1900, tstruct.tm_mon + 1, tstruct.tm_mday, tstruct.tm_hour, 15, 0);
                        }
                    } catch(...) {
                    }
                    i++;
